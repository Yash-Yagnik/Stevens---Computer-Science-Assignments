...
static int image_num_blocks(struct blkdev *dev)
{
	...
	//return the number of blocks which is in nblks
	return ((struct image_dev*) (dev->private))->nblks;
}
...
static int image_write(struct blkdev * dev, int first_blk, int nblks, void *buf)
{
	...
	//super block warning
	if (first_blk == 0) {
		fprintf(stderr, "warning: writing to the superblock\n");
	}
...
static void image_close(struct blkdev *dev)
{
	...
	if (im->fd != -1) {
		if(close(im->fd) == -1) {
			fprintf(stderr, "can't close image %s: %s\n", im->path, strerror(errno));
		} else {
			free(im->path);
			free(im);
			free(dev);
