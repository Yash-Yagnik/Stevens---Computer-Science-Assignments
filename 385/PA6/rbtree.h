...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        Node<K, V> *x, *y;
        Node<K, V> *z = new Node<K, V>(key, key_value.second); // Make new node
        if (z == nullptr){ // Check if null and if so return nothing
            return;
        }
        if (it != end()) { // Initalize the tree
            x = it.node_ptr;
            y = x->parent;
        }
        else {
            x = root_; // Initalize the root
            y = nullptr;
        }
        ...
        while (x != nullptr){
            ...
            if (z->key() < x->key()){
                x = x->left; // If the value being inserted is less than the node we are at it becomes the left child
            }
            else if (z->key() > x->key()){
                x = x->right; // If the value being inserted is greater than the node we are at it becomes the right child
            }
            else{
                std::ostringstream out;
                out << "Attempt to insert duplicate key '" << key << "'.";
                //out.str("Attempt to insert duplicate key ");
                delete z;
                throw tree_exception(out.str()); // Throws an exception if the node given to input is already seen in the tree
        ...
        if (y == nullptr){
            root_ = z; // Make node the root
            size_++;
        }
        else if (z->key() < y->key()){
            y->left = z; // Make node the left child of y
            size_++;
        }
        else{
            y->right = z; // Make node the right child of y
            size_++;
        }
        ...
        z->right = nullptr;
        z->color = RED;
        insert_fixup(z); // The tree could be all messed up after the insertion of one node, so send it to insert_fixup to do tree rotations
    }
...
private:
    ...
    void delete_tree(Node<K, V> *n) {
        ...
        delete_tree(n->right); // Uses recurion to break the tree into a left and right subtree then delete the nodes individually
        delete_tree(n->left);
        delete n;
        return;
    ...
    int height(Node<K, V> *node) const {
        if (node == nullptr){ // If no nodes exist then -1 is the height
            return -1;
        }
        else{
            if (node->left == nullptr){
                return 1 + height(node->right); // If left side is empty go down the right subtree
            }
            else if (node->right == nullptr){
                return 1 + height(node->left); // If right side is empty go down the left subtree
            }
            else{
                return 1 + std::max(height(node->left), height(node->right)); // If both subtrees are not empty then go down both subtrees
            }
        }
    ...
    size_t leaf_count(Node<K, V> *node) const {
        ...
        if (node == nullptr){ // If tree is empty then 0 leafs
            return 0;
        }
        else if (node->right == nullptr && node->left == nullptr){ // If the nodes doesn't have both a left and right child then it is a node
            total_leafs++;
        }
        else if (node->right == nullptr){
            total_leafs += leaf_count(node->left); // Right child doesn't exist go to the left subtree
        }
        else if (node->left == nullptr){
            total_leafs += leaf_count(node->right); // Left child doesn't exist go to the right subtree
        }
        else{
            total_leafs += (leaf_count(node->left) + leaf_count(node->right)); // Both exist so go to both subtrees
        }
        return total_leafs;
    }
        ...
        if (node == nullptr){
            ...
            if (node->right != nullptr || node->left != nullptr){
                ...
                if (node->right == nullptr){
                    total_internal_nodes += internal_node_count(node->left);
                }
                else if (node->left == nullptr) {
                    total_internal_nodes += internal_node_count(node->right);
                }
                else{
                    total_internal_nodes += internal_node_count(node->left) + internal_node_count(node->right);
                }
            }
        }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
        // We want to find all the nodes that are NOT empty on the given level, so subtract the level till it becomes 0 so we know we are at that level
    ...
    size_t null_count(Node<K, V> *node) const {
        if (node == nullptr){
            return 1; // If the node is null then + 1
        }
        return null_count(node->left) + null_count(node->right); // Traverse down the tree to find each null node
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        size_t sum_levels_total = 0;
        while (level <= (size_t)height()){
            sum_levels_total += level * width(root_, level); // Go to each level and find the width and mulitply the level and width
            level++;
        }
        return sum_levels_total;
    }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
        // For each node that is null multiply it by the level that's why level * 1 = level, then use recursion to
        if (node == nullptr){
            return level;
        }
