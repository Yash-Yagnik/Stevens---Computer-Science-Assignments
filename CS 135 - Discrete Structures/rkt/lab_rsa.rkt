...
(define (generate p q e)
  (cons (list (* p q) e) (list (list (* p q) (mod-inverse e (lcm (- p 1) (- q 1))))))
 )
...
(define (mod-exp b e m)
  (cond [(equal? e 0) 1]
        [(equal? (modulo e 2) 0) (modulo (expt (mod-exp b (/ e 2) m) 2) m)]
        [(equal? (modulo e 2) 1) (modulo (* (mod-exp b (- e 1) m) b) m)]
        )
)
...
(define (encrypt plaintext key)
  (reverse (encrypt-helper (string->list plaintext) key '()))
  
)

(define (encrypt-helper list key return)
  (cond [(null? list) return]
        [else (encrypt-helper (cdr list) key (cons (mod-exp (char->integer (car list)) (car (cdr key)) (car key)) return))])
)
...
(define (decrypt ciphertext key)
 (list->string  (reverse (decrypt-helper ciphertext key '())))
)

(define (decrypt-helper list key return)
  (cond [(null? list) return]
        [else (decrypt-helper (cdr list) key (cons (integer->char (mod-exp (car list) (car (cdr key)) (car key))) return))])
)
