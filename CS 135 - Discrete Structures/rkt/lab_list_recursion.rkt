...
(define (rotate c)
  (define list1 c)
  (define list2 (list (car (reverse c)) (* -1 (car c))))
  (define list3 (list (* -1 (car c)) (* -1 (car (reverse c)))))
  (define list4 (list (* -1 (car (reverse c))) (car c)))
...
(define (combine-courses s1 s2)
  (append (car (reverse (cdr (cdr (cdr (cdr s1)))))) (car (reverse (cdr (cdr (cdr (cdr s2)))))))
)
...
;; 1 PTS
(define (sign x)
  (cond [(> x 0) "positive"] [(< x 0) "negative"] [(= x 0) "zero"])
)
...
;; 3 PTS
(define (subfact n)
  (cond [(= n 1) 0] [(= n 2) 1] [(= (modulo n 2) 0) (+ (* n (subfact (+ n -1))) 1)] [(= (modulo n 2) 1) (+ (* n (subfact (+ n -1))) -1)])
)
