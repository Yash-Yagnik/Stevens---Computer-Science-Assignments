...
(define (help x S L)
  (cond [(null? S) L]
        [else (help x (cdr S) (append L (list (list x (car S)))))])

  )

(define (compose S R)
  (define (myCompose S R L)
   (cond [(equal? (null? S) '()) '()]
         [(null? R) '()]
         [else (myCompose  S (cdr R) (append L (help (car (car R)) (relate (list (cdr (car (cdr R)))) S) '())))])
...
(define (equivalence? n R)
  ...
  (cond [(and (reflexive? n R) (symmetric? R) (transitive? R)) #t]
        [else #f]
        )
