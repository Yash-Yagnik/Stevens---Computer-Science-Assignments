...
(define (divides? a b)
  (cond
    [(and (equal? a 0) (equal? b 0)) #t][(and (not (equal? a 0)) (equal? (modulo b a) 0)) #t][else #f])
)
...
(define (myWaterjug cA cB fA fB g)
    (cond [(eq? fB g) (cons (list fA fB) '())]
          [(eq? fB cB) (cons (list fA fB)(myWaterjug cA cB fA 0 g))]
          [(eq? fA 0) (cons (list fA fB)(myWaterjug cA cB cA fB g ))]
          [(> (+ fA fB) cB) (cons (list fA fB)(myWaterjug cA cB (- fA (- cB fB)) cB g))]
          [(<= (+ fA fB) cB) (cons (list fA fB) (myWaterjug cA cB 0 (+ fB fA) g))])
) 
(define (waterjug cA cB g)
  (myWaterjug cA cB 0 0 g)
)
...
(define (factorize-helper n p)
    (cond
      [(equal? n 0) (list 0)]
      [(equal? n 1) '()]
      [(equal? (modulo n p) 0) (append (list p) (factorize-helper (quotient n p) 2))]
      [else (factorize-helper n (+ p 1))])
)
