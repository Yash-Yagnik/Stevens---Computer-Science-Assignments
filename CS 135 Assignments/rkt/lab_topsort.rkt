...
(define (my-indegree E v c)
  (cond [(null? E) c]
        [(equal? (car (cdr (car E))) v) (my-indegree (cdr E) v (+ 1 c))]
        [else (my-indegree (cdr E) v c)]
    )
)
...
;; 5 PTS
(define (find-top G)
  (my-find-top (get-edges G) (get-vertices G))
  )


(define (my-find-top e v)
  (cond
    [(null? v) (topsort-error)]
    [(equal? (indegree e (car v)) 0) (car v)]
    [else (my-find-top e (cdr v))]
)
)
...
(define (my-topsort G list)
  (cond
    [(null? (get-vertices G)) (reverse list)]
    [else (my-topsort (pop-vertex G (find-top G)) (cons (find-top G) list))]
  ) 
